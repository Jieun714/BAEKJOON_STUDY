 ...
 *      만약 x가 0이 아니라면 배열에 x라는 값을 넣는(추가하는) 연산이고, x가 0이라면 배열에서 절댓값이 가장 작은 값을 출력하고 그 값을 배열에서 제거하는 경우이다. 입력되는 정수는 -2^31보다 크고, 2^31보다 작다.
...
public class baekjoon_11286 {
    public static void main(String[] args) throws IOException {
        ...
        for(int i=0; i<N; i++){
            ...
            if(num == 0){
                if(queue.isEmpty()){ //큐가 비어 있으면 0
                    sb.append(0+"\n");
                } else { //비어 있지 않으면 큐의 front값 출력
                    sb.append(queue.poll() + "\n");
                }
            } else { //새로운 데이터를 우선순위 큐에 삽입
                queue.add(num);
            }
        }
